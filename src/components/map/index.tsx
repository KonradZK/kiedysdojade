...
const MapFocusHandler = ({ 
  ...
  const map = useMap();
  const lastFocusRef = useRef<{ start: StopGroup | null; end: StopGroup | null }>({
    start: null,
    end: null,
  });
...
const Map = ({ stops, lines, selectedStart, selectedEnd }: MapProps) => {
  const hasRoute = stops.length > 0;
  
  const route = useMemo(() => {
    const defaultPositions = stops.map((stop) => ({ lat: stop.lat, lng: stop.lon }));

    const linesToRender = lines.length > 0 ? lines : [];

    return (
      <>
        <CustomMarkers positions={defaultPositions} />
        {linesToRender.length > 0 ? (
          linesToRender.map((line, idx) => {
            const positions = line.shape.length > 0
